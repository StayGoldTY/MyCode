...
Begin {C62A69F0-16DC-11CE-9E98-00AA00574A4F} Criteria 
   Caption         =   "Create List of Columns Table in Word"
   ClientHeight    =   5628
   ClientLeft      =   36
   ClientTop       =   420
   ClientWidth     =   7020
...
' Table heading width
Dim tblAttrWidth(20)  As Integer
...
Dim colAttrWidth(20)  As Integer
...
' Create table in Word
Private Sub CreateTableInWord()
   On Error Resume Next
   ' Get current document
   Set oDoc = ActiveDocument
   ' Create table
   Dim oTable As Word.Table
   Set oTable = InsertTable()
   ' Export header
   ExportHeader oTable
   ' Export model
   ExportModel oTable
...
' Insert a table
Private Function InsertTable() As Word.Table
   ' Insert title
   ...
   With myRange
      ...
      ' Insert title
      .InsertParagraph
      .InsertBefore "List of Columns"
      .Style = wdStyleHeading1
      .Collapse Direction:=wdCollapseEnd
      ' Insert criteria
      If TableCode.Text <> "" Then
         .InsertParagraph
         .InsertBefore "Table code starts with: " + TableCode.Text
         .Collapse Direction:=wdCollapseEnd
      End If
      If ColumnCode.Text <> "" Then
         .InsertParagraph
         .InsertBefore "Column code starts with: " + ColumnCode.Text
         .Collapse Direction:=wdCollapseEnd
      End If
      If ColumnTypes.Text <> "" Then
         .InsertParagraph
         .InsertBefore "Column data type: " + ColumnTypes.Text
         .Collapse Direction:=wdCollapseEnd
      End If
   End With
   ...
   ' Create a table
   Set InsertTable = oDoc.Tables.Add(myRange, NumRows:=nbRows, NumColumns:=nbCols)
End Function

' Create headers in Word
Private Sub ExportHeader(oTable)
   On Error Resume Next
   Dim noCol, n
   ...
   For n = 0 To nbTblAttr - 1
      If tblAttrExport(n) Then
         Set oCell = oTable.Cell(1, noCol + 1)
         oCell.Range.InsertAfter tblAttrHead(n)
         noCol = noCol + 1
      End If
   Next
   ...
   For n = 0 To nbColAttr - 1
      If colAttrExport(n) Then
         Set oCell = oTable.Cell(1, noCol + 1)
         oCell.Range.InsertAfter colAttrHead(n)
         noCol = noCol + 1
      End If
   Next
End Sub

' Export model in Word
Private Sub ExportModel(oTable)
   On Error Resume Next
   Dim model As PdPDM.model
   Set model = GetModel()
   If Not model Is Nothing Then
      ...
      For Each tbl In model.Tables
         If VerifyTableCriteria(tbl) Then
            For Each col In tbl.Columns
               If VerifyColumnCriteria(col) Then
                  ExportRow tbl, oTable, col, nbRow
                  nbRow = nbRow + 1
               End If
            Next
         End If
...
' Export a column as a row in Word
Sub ExportRow(tbl, oTable, col, nbRow)
   On Error Resume Next
   Dim noCol, n
   Dim oCell As Word.Cell
   noCol = 0
   ...
   For n = 0 To nbTblAttr - 1
      If tblAttrExport(n) And tbl.HasAttribute(tblAttr(n)) Then
         Set oCell = oTable.Cell(nbRow, noCol + 1)
         oCell.Range.InsertAfter tbl.GetAttributeText(tblAttr(n))
         noCol = noCol + 1
      End If
   Next
   ' Export column attributes header
   For n = 0 To nbColAttr - 1
      If colAttrExport(n) And col.HasAttribute(colAttr(n)) Then
         Set oCell = oTable.Cell(nbRow, noCol + 1)
         oCell.Range.InsertAfter col.GetAttributeText(colAttr(n))
         noCol = noCol + 1
      End If
   Next
End Sub
...
' Auto format the table
Sub FormatTable(oTable)
   oTable.AutoFormat Format:=wdTableFormatProfessional, ApplyBorders:=True, ApplyFont:=True, ApplyColor:=True
   ...
   For n = 0 To nbTblAttr - 1
      If tblAttrExport(n) Then
         nbCols = nbCols + 1
         oTable.Columns(nbCols).Cells.SetWidth ColumnWidth:=tblAttrWidth(n), RulerStyle:=wdAdjustSameWidth
      End If
   Next
   For n = 0 To nbColAttr - 1
      If colAttrExport(n) Then
         nbCols = nbCols + 1
         oTable.Columns(nbCols).Cells.SetWidth ColumnWidth:=colAttrWidth(n), RulerStyle:=wdAdjustFirstColumn
      End If
   Next
End Sub
